...
export default async function DriverReportPage({
  ...
  searchParams: Promise<{ period?: string; from?: string; to?: string; company?: string; fleet?: string }>;
}) {
  const { id } = await params;
  const query = await searchParams;
  const period = normalizePeriod(query.period);
  const supabase = await createClient();
  const scope = parseCompanyScope(query.company ?? query.fleet, await getLoadCompanies());
  if (!scope) notFound();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <Link href={company ? `/reports?company=${encodeURIComponent(company)}` : "/reports"} className="text-sm text-zinc-500 underline-offset-2 hover:underline">
            ← Back to reports
          </Link>
          <h1 className="mt-2 text-2xl font-semibold text-zinc-950">{driver.name ?? "Driver"}</h1>
          <p className="text-sm text-zinc-600">{companyScopeLabel(scope)} · Weekly payroll and financials.</p>
        </div>
