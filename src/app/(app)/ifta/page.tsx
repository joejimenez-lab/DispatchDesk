    ...
    <div className="space-y-6">
                ...
                <tr key={trip.id} className="hover:bg-zinc-50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex flex-wrap justify-end gap-2">
                      {trip.loads ? <Link href={`/loads/${trip.loads.id}`} className="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-medium text-blue-800">Load {trip.loads.load_number}</Link> : null}
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <details className="group w-full sm:w-auto">
            <summary className="cursor-pointer list-none rounded-[10px] bg-blue-600 px-4 py-2.5 text-center text-sm font-semibold text-white shadow-sm hover:bg-blue-700">+ Add fuel purchase</summary>
            <div className="mt-4 rounded-xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-5 lg:absolute lg:left-1/2 lg:z-10 lg:w-[min(68rem,calc(100vw-2rem))] lg:-translate-x-1/2 lg:shadow-xl">
              <div className="mb-3 flex justify-end"><DetailsCloseButton /></div>
              <IftaFuelForm action={addIftaFuelPurchase} trucks={fuelTrucks} />
            </div>
          </details>
        </div>
        <div className="overflow-x-auto rounded-lg border border-zinc-200 bg-white">
            ...
            <tbody className="divide-y divide-zinc-100">
              {purchases.map((purchase) => (
                <tr key={purchase.id} className="hover:bg-zinc-50">
                  ...
                  <td className="px-4 py-3 text-right">
                    {(() => {
                      const group = purchase.posted_bookkeeping?.[0];
                      const sourceGroup = purchase.source_bookkeeping;
                      return (
                        <div className="flex flex-wrap justify-end gap-2">
                          {sourceGroup ? (
                            <Link href={`/bookkeeping?unit=${purchase.unit_id ?? ""}&from=${purchase.purchase_date}&to=${purchase.purchase_date}`} className="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-medium text-blue-800">
                              Source · {sourceGroup.bookkeeping_receipts.length} receipt{sourceGroup.bookkeeping_receipts.length === 1 ? "" : "s"}
                            </Link>
                          ) : null}
                          {group ? (
                            <Link href={`/bookkeeping?unit=${purchase.unit_id ?? ""}&from=${purchase.purchase_date}&to=${purchase.purchase_date}`} className="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-medium text-blue-800">
                              Bookkeeping · {group.bookkeeping_receipts.length} receipt{group.bookkeeping_receipts.length === 1 ? "" : "s"}
                            </Link>
                          ) : null}
                          {group ? (
                            <details className="text-left">
                              <summary className="cursor-pointer list-none rounded-md border border-zinc-300 px-3 py-2 text-sm font-medium">Edit</summary>
                              <div className="mt-3 rounded-lg border border-zinc-200 bg-zinc-50 p-4 lg:absolute lg:left-1/2 lg:z-10 lg:w-[min(68rem,calc(100vw-2rem))] lg:-translate-x-1/2 lg:shadow-xl">
                                <IftaFuelForm action={updateIftaFuelPurchase.bind(null, purchase.id, group.id)} trucks={fuelTrucks} purchase={purchase} />
                              </div>
                            </details>
                          ) : null}
                          <ActionForm action={(sourceGroup ? deleteImportedIftaFuelPurchase : deleteIftaFuelPurchase).bind(null, purchase.id)} successMessage={false}>
                            <ConfirmSubmitButton message={sourceGroup ? "Remove this posted IFTA purchase? Its source Bookkeeping transaction will be preserved." : "Delete this fuel purchase and its linked Bookkeeping transaction?"} variant="secondary">Delete</ConfirmSubmitButton>
                          </ActionForm>
                        </div>
                      );
                    })()}
