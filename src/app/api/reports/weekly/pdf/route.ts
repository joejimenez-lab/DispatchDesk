...
import { companyScopeLabel, companyScopeSlug, resolveExportCompanyScope } from "@/lib/company-scope";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const scope = await resolveExportCompanyScope(auth.supabase, searchParams.get("company") ?? searchParams.get("fleet"));
    ...
    const pdf = await renderWeeklySummaryPdf(summaries, range, undefined, companyScopeLabel(scope));
    const stamp = new Date().toISOString().slice(0, 10);

    return new NextResponse(new Uint8Array(pdf), {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="dispatchdesk-financial-summary-${companyScopeSlug(scope)}-${stamp}.pdf"`,
