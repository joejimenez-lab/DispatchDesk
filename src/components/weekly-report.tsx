...
import { UNASSIGNED_COMPANY } from "@/lib/company-scope";
...
function WeekCard({ summary, linkDriver }: { summary: WeeklyDriverFinancialSummary; linkDriver: boolean }) {
  const driverHeading =
    linkDriver && summary.driverId ? (
      <Link href={`/reports/drivers/${summary.driverId}?company=${encodeURIComponent(summary.carrierCompany ?? UNASSIGNED_COMPANY)}`} className="underline-offset-2 hover:underline">
        {summary.driverName}
      </Link>
    ) : (
      summary.driverName
  ...
  return (
    <div className="rounded-lg border border-zinc-200 bg-white">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-zinc-200 p-5">
        <div>
          <h2 className="text-lg font-semibold text-zinc-950">{driverHeading} · {summary.carrierCompany ?? "Unassigned"}</h2>
          ...
          <tbody className="divide-y divide-zinc-100">
            {summary.loads.map((load) => (
              <tr key={load.id} className="hover:bg-zinc-50">
                ...
                <td className="px-4 py-3 text-zinc-700">{load.carrierCompany ?? "Unassigned"}<span className="block text-xs text-zinc-500">Hauled by {load.haulingFleet ?? "Unassigned"}</span></td>
